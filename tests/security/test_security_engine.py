...
class TestSecurityEngine:
    ...
    def security_engine_setup(self, tmp_path, monkeypatch):
        ...
        shutil.rmtree(mock_main_backup_path.parent / ".backy-secrets", ignore_errors=True)
    ...
    def test_init_success(self, security_engine_setup, monkeypatch):
        ...
        monkeypatch.setattr(SecurityEngine, '_load_public_key', lambda self: ("key", "v1"))
    ...
    def test_init_not_have_keys(self, security_engine_setup, monkeypatch):
        ...
        monkeypatch.setattr(SecurityEngine, '_generate_pub_and_priv_key', mock_create_keys)
        monkeypatch.setattr(SecurityEngine, '_load_public_key', lambda self: ("key", "v1"))
        engine = SecurityEngine(config)
    ...
    def test_check_two_keys_exist_method(self, security_engine_setup, caplog, monkeypatch):
        """
        ...
        with caplog.at_level("INFO"):
            ...
            assert "Public and private key files found in the secret path" in caplog.text

    def test_check_two_keys_not_exist_method(self, security_engine_setup, monkeypatch, caplog):
        """
        ...
        with caplog.at_level("INFO"):
            ...
            assert "No public or private key files found in the secret path" in caplog.text

    def test_generate_pub_and_priv_key_success(self, security_engine_setup, monkeypatch):
        """
    ...
    def test_generate_pub_and_priv_key_failure(self, security_engine_setup, monkeypatch, mocker, caplog):
        """
        Test that generate_pub_and_priv_key handles errors during key generation.
        """
        config, _ = security_engine_setup
        monkeypatch.setattr(SecurityEngine, '_check_keys_exist', lambda self: False)
        mocker.patch("security.security_engine.rsa.generate_private_key", side_effect=Exception("Mock key gen error"))
        with caplog.at_level("ERROR"):
            with pytest.raises(RuntimeError) as excinfo:
                SecurityEngine(config)
            assert "Failed to generate public and private keys" in str(excinfo.value)
            assert "Error generating public and private keys: Mock key gen error" in caplog.text
    ...
    def test_load_public_key_success(self, security_engine_setup, monkeypatch):
        ...
        with open(old_key_path, "wb") as f:
            f.write(rsa.generate_private_key(public_exponent=65537, key_size=2048).public_key().public_bytes(
    ...
    def test_load_public_key_file_not_found(self, security_engine_setup, monkeypatch, caplog):
        """
    ...
    def test_load_public_key_failure(self, security_engine_setup, monkeypatch, caplog, mocker):
        """
        ...
        # Ensure _check_keys_exist returns True to trigger loading
        monkeypatch.setattr(SecurityEngine, '_check_keys_exist', lambda self: True)
        mocker.patch("security.security_engine.open", side_effect=Exception("Mock load key error"))
